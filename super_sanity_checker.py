...
def make_test_input_file() -> Path:
    with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', delete=False) as input_file:
        input_file.write('LENGTH 1000\n')
        input_file.write('DEVICE 1\n')
        input_file.write('DEVICE 2\n')
        input_file.write('DEVICE 3\n')
        input_file.write('DEVICE 4\n')
        input_file.write('DEVICE 5\n')
        input_file.write('DEVICE 6\n')
        input_file.write('DEVICE 7\n')
        input_file.write('PROPAGATE 1 2 100\n')
        input_file.write('PROPAGATE 1 3 75\n')
        input_file.write('PROPAGATE 2 3 50\n')
        input_file.write('PROPAGATE 3 4 150\n')
        input_file.write('PROPAGATE 4 1 125\n')
        input_file.write('PROPAGATE 5 1 10\n')
        input_file.write('PROPAGATE 6 7 900\n')
        input_file.write('PROPAGATE 7 6 40\n')
        input_file.write('ALERT 1 Badness 200\n')
        input_file.write('ALERT 3 rizzwizz 500\n')
        input_file.write('ALERT 5 zaza 5\n')
        input_file.write('ALERT 6 zestyjalapeño 50\n')
        input_file.write('CANCEL 1 Badness 600\n')
        input_file.write('CANCEL 5 zaza 0\n')
        input_file.write('CANCEL 7 zestyjalapeño 949\n')
...
def make_test_lines(input_path: Path) -> list[TestInputLine | TestOutputLines]:
    return [
        ...
        TestOutputLines('@0: #5 SENT CANCELLATION TO #1: zaza', timeout=10.0),
        TestOutputLines('@5: #5 SENT ALERT TO #1: zaza', timeout=10.0),
        TestOutputLines('@10: #1 RECEIVED CANCELLATION FROM #5: zaza',
                        '@10: #1 SENT CANCELLATION TO #2: zaza',
                        '@10: #1 SENT CANCELLATION TO #3: zaza', timeout=10.0),
        TestOutputLines('@15: #1 RECEIVED ALERT FROM #5: zaza', timeout=10.0),
        TestOutputLines('@50: #6 SENT ALERT TO #7: zestyjalapeño', timeout=10.0),
        TestOutputLines('@85: #3 RECEIVED CANCELLATION FROM #1: zaza',
                        '@85: #3 SENT CANCELLATION TO #4: zaza', timeout=10.0),
        TestOutputLines('@110: #2 RECEIVED CANCELLATION FROM #1: zaza',
                        '@110: #2 SENT CANCELLATION TO #3: zaza', timeout=10.0),
        TestOutputLines('@160: #3 RECEIVED CANCELLATION FROM #2: zaza', timeout=10.0),
        TestOutputLines('@200: #1 SENT ALERT TO #2: Badness',
                        '@200: #1 SENT ALERT TO #3: Badness', timeout=10.0),
        TestOutputLines('@235: #4 RECEIVED CANCELLATION FROM #3: zaza',
                        '@235: #4 SENT CANCELLATION TO #1: zaza', timeout=10.0),
        TestOutputLines('@275: #3 RECEIVED ALERT FROM #1: Badness',
                        '@275: #3 SENT ALERT TO #4: Badness',
                        timeout=10.0),
        TestOutputLines('@300: #2 RECEIVED ALERT FROM #1: Badness',
                        '@300: #2 SENT ALERT TO #3: Badness', timeout=10.0),
        TestOutputLines('@350: #3 RECEIVED ALERT FROM #2: Badness',
                        '@350: #3 SENT ALERT TO #4: Badness', timeout=10.0),
        TestOutputLines('@360: #1 RECEIVED CANCELLATION FROM #4: zaza', timeout=10.0),
        TestOutputLines('@425: #4 RECEIVED ALERT FROM #3: Badness',
                        '@425: #4 SENT ALERT TO #1: Badness', timeout=10.0),
        TestOutputLines('@500: #4 RECEIVED ALERT FROM #3: Badness',
                        '@500: #4 SENT ALERT TO #1: Badness',
                        '@500: #3 SENT ALERT TO #4: rizzwizz', timeout=10.0),
        TestOutputLines('@550: #1 RECEIVED ALERT FROM #4: Badness',
                        '@550: #1 SENT ALERT TO #3: Badness',
                        '@550: #1 SENT ALERT TO #2: Badness', timeout=10.0),
        TestOutputLines('@600: #1 SENT CANCELLATION TO #2: Badness',
                        '@600: #1 SENT CANCELLATION TO #3: Badness', timeout=10.0),
        TestOutputLines('@625: #1 RECEIVED ALERT FROM #4: Badness',
                        '@625: #3 RECEIVED ALERT FROM #1: Badness',
                        '@625: #3 SENT ALERT TO #4: Badness', timeout=10.0),
        TestOutputLines('@650: #2 RECEIVED ALERT FROM #1: Badness',
                        '@650: #2 SENT ALERT TO #3: Badness',
                        '@650: #4 RECEIVED ALERT FROM #3: rizzwizz',
                        '@650: #4 SENT ALERT TO #1: rizzwizz', timeout=10.0),
        TestOutputLines('@675: #3 RECEIVED CANCELLATION FROM #1: Badness',
                        '@675: #3 SENT CANCELLATION TO #4: Badness', timeout=10.0),
        TestOutputLines('@700: #2 RECEIVED CANCELLATION FROM #1: Badness',
                        '@700: #2 SENT CANCELLATION TO #3: Badness',
                        '@700: #3 RECEIVED ALERT FROM #2: Badness', timeout=10.0),
        TestOutputLines('@750: #3 RECEIVED CANCELLATION FROM #2: Badness', timeout=10.0),
        TestOutputLines('@775: #4 RECEIVED ALERT FROM #3: Badness',
                        '@775: #4 SENT ALERT TO #1: Badness',
                        '@775: #1 RECEIVED ALERT FROM #4: rizzwizz',
                        '@775: #1 SENT ALERT TO #2: rizzwizz',
                        '@775: #1 SENT ALERT TO #3: rizzwizz', timeout=10.0),
        TestOutputLines('@825: #4 RECEIVED CANCELLATION FROM #3: Badness',
                        '@825: #4 SENT CANCELLATION TO #1: Badness', timeout=10.0),
        TestOutputLines('@850: #3 RECEIVED ALERT FROM #1: rizzwizz',
                        '@850: #3 SENT ALERT TO #4: rizzwizz', timeout=10.0),
        TestOutputLines('@875: #2 RECEIVED ALERT FROM #1: rizzwizz',
                        '@875: #2 SENT ALERT TO #3: rizzwizz', timeout=10.0),
        TestOutputLines('@900: #1 RECEIVED ALERT FROM #4: Badness', timeout=10.0),
        TestOutputLines('@925: #3 RECEIVED ALERT FROM #2: rizzwizz',
                        '@925: #3 SENT ALERT TO #4: rizzwizz', timeout=10.0),
        TestOutputLines('@949: #7 SENT CANCELLATION TO #6: zestyjalapeño', timeout=10.0),
        TestOutputLines('@950: #1 RECEIVED CANCELLATION FROM #4: Badness',
                        '@950: #7 RECEIVED ALERT FROM #6: zestyjalapeño', timeout=10.0),
        TestOutputLines('@989: #6 RECEIVED CANCELLATION FROM #7: zestyjalapeño',
                        '@989: #6 SENT CANCELLATION TO #7: zestyjalapeño', timeout=10.0),
        TestOutputLines('@1000: END', timeout=10.0),
        TestEndOfOutput(2.0)
    ]
